...
export default function Card({
  ...
  const variants: Record<CardVariant, string> = {
    elevated: 'bg-white shadow-md transition-shadow hover:shadow-lg dark:bg-gray-800',
    outlined: 'border border-gray-200 bg-white dark:border-gray-700 dark:bg-gray-800',
    filled: 'bg-gray-100 dark:bg-gray-800',
  }

  return (
    <div className={`overflow-hidden rounded-xl ${variants[variant]}`}>
      {image && <img src={image} alt={imageAlt ?? ''} className="h-48 w-full object-cover" />}
      <div className="p-5">
        {title && (
